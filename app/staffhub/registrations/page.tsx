...
export default function AdminRegistrationsPage() {
  ...
  useEffect(() => {
    if (!authLoading) {
      if (!user) { router.push('/staffhub/login'); return; }
  ...
  async function handleApprove(id: string) {
    ...
    try {
      ...
      setFeedback({ id, ok: true, msg: `School account created! → /staffhub/schools/${data.schoolId}` });
      setApprovingId(null);
      fetchRegistrations();
    } catch (err: any) {
      ...
      <div className="min-h-screen bg-[var(--wrife-bg)] py-8">
        <div className="mx-auto max-w-5xl px-4">
          <div className="mb-6 flex items-center justify-between gap-4 flex-wrap">
            <div>
              <Link href="/staffhub" className="text-sm text-[var(--wrife-blue)] hover:underline mb-1 inline-block">
                ← Back to Dashboard
              </Link>
                ...
                <div key={reg.id} className="bg-white rounded-2xl border border-[var(--wrife-border)] overflow-hidden">
                  ...
                  >
                    ...
                    {reg.status === 'approved' && reg.school_id && (
                      <Link
                        href={`/staffhub/schools/${reg.school_id}`}
