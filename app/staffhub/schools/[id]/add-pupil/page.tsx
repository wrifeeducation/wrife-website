...
export default function AddPupilPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => {
        router.push(`/staffhub/schools/${schoolId}/users`);
      }, 2000);
    } catch (err: any) {
    ...
    <>
      ...
      <div className="min-h-screen bg-[var(--wrife-bg)] py-8">
        <div className="mx-auto max-w-md px-4">
          <div className="mb-6">
            <Link href={`/staffhub/schools/${schoolId}/users`} className="text-sm text-[var(--wrife-blue)] hover:underline mb-2 inline-block">
              ← Back to Users
            </Link>
            <h1 className="text-2xl font-extrabold text-[var(--wrife-text-main)]">Add Pupil</h1>
            <p className="text-sm text-[var(--wrife-text-muted)] mt-1">
              Create a new pupil account for this school
            </p>
          </div>

          {success ? (
            <div className="bg-green-50 border border-green-200 rounded-xl p-6 text-center">
              <p className="text-green-700 font-semibold">Pupil added successfully!</p>
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-soft border border-[var(--wrife-border)] p-6 space-y-4">
              ...
              >
                {submitting ? 'Adding Pupil...' : 'Add Pupil'}
              </button>
            </form>
          )}
        </div>
