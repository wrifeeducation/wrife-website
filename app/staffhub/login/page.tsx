...
export default function AdminLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (authUser) {
      ...
      window.location.replace('/staffhub');
    } else {
        ...
        <div className="bg-slate-800 rounded-2xl border border-slate-700 p-6 shadow-2xl shadow-blue-500/10">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="text-right">
              <Link href="/staffhub/reset-password" className="text-sm font-medium text-slate-300 hover:text-[var(--wrife-blue)] transition">
                Forgot password?
              </Link>
            </div>
