...
- **Authentication:** 
    - **Staff:** Supabase handles authentication for teachers and admins. Admin APIs use `requireAdmin()`.
    - **Pupil Accounts (TTRockstars-style):** Pupils log in without email using class code + username + 4-digit PIN. Credentials are generated by teachers when adding pupils to classes. Uses bcrypt hashing and httpOnly session cookies (8-hour TTL).
    - **Pupil Tables:** `pupils` table stores pupil accounts with `class_id`, `username`, `password_hash`, `is_active`, `last_login_at`. `pupil_sessions` tracks active sessions. `pupil_activity_log` records login/logout events.
    - **Class Codes:** Auto-generated unique 8-character codes (e.g., "BLU52341") with uniqueness constraint and collision-safe generation.
...
- **Class Management:** Teachers can create classes with auto-generated class codes, add pupils with auto-generated credentials (username + PIN), view rosters, print login cards, and assign activities. APIs at `/api/classes` and `/api/classes/[id]/pupils`.
